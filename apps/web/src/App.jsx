 ...
 return <main className="page dentist-dashboard"><section className="wide">
   ...
   {!data?<section className="card">Carregando painel…</section>:<>
     ...
     {tab==="patients"&&<section className="card">
       ...
       {data.patients.length===0?<div className="empty">Nenhum paciente cadastrado.</div>:<div className="patient-registry">{data.patients.map(x=><div className="patient-registry-row" key={x.id}><div className="patient-registry-main"><strong>{x.name}</strong><small>{x.phone||"Sem telefone"}{x.birthDate?" • "+new Date(x.birthDate).toLocaleDateString("pt-BR"):""}</small>{(x.referringDentistName||x.referringDentistCro||x.referringDentistEmail)&&<small>Dentista: {x.referringDentistName||"Não informado"}{x.referringDentistCro?" • CRO "+x.referringDentistCro:""}{x.referringDentistEmail?" • "+x.referringDentistEmail:""}</small>}</div><button className="secondary compact" onClick={()=>{setSelectedPatient(x.id);setPatientMode("existing");setOut(null);goDentistTab("new")}}>Criar pedido</button></div>)}</div>}
     </section>}
   </>}
 </section></main>
}
